...
class Evaluator:
    ...
    def _lazy_init(self):
        ...
        # and counts against the winrate. The previous value (4000 on 16x16
        # maps) let stalled games run twice as long as the evaluation
        # protocol allows, converting would-be draws into wins and inflating
        # the monitored winrate that gates early stopping.
    ...
    def evaluate(self, train_model, step=0):
        """
        Plays one game in each of eval_workers parallel envs and counts each
        env's first completed game (running until all have finished, so slow
        games and full-length draws are included). Returns winrate in
        [0, 1]. Games are capped at eval_max_steps (default 2000, matching
        the evaluation protocol); games undecided at the cap are draws, and
        draws and losses both count against the winrate.
        ...
        envs = self.envs
        n = self.args.eval_workers
        # Count each env's FIRST completed game, and run until every env has
        # finished one. Counting the first eval_games completions across all
        # envs (as before) is length-biased: with many parallel envs, fast
        # games (mostly wins) fill the quota before slow games (losses) and
        # especially draws (always the full episode cap) can finish, so the
        # monitored winrate only sampled the quickest games. The effective
        # sample size per check is eval_workers games.
        counted = [False] * n
        wins = games = 0
...
def experiment(args):
    ...
    while step < args.max_steps:
        ...
        if args.eval_every > 0 and step >= next_eval_at:
            ...
            print(f"[eval] step={step:,} | playing {args.eval_workers} games "
